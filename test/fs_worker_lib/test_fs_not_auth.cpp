...
FsWorkerNotAuthUsr create_fs_not_auth_usr(const fs::path &root_path) {
    FsWorkerNotAuthUsr fs_worker(root_path);
...
TEST(FsNotAuthUsr, CreateValid) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/fs_sub_worker_not_auth_usr/");
    FsWorkerNotAuthUsr fs_worker1(root_path);
    EXPECT_EQ(root_path, fs_worker1.get_root());
    EXPECT_FALSE(static_cast<bool>(fs_worker1.err_code));
    EXPECT_TRUE(fs_worker1.get_sort_dirs().empty());

    FsWorkerNotAuthUsr fs_worker2(fs_worker1);
    EXPECT_EQ(fs_worker2, fs_worker1);

    FsWorkerNotAuthUsr fs_worker3(std::move(fs_worker1));
    EXPECT_EQ(fs_worker3, fs_worker2);

    std::error_code ec;
    fs::remove_all(root_path, ec);
    ASSERT_FALSE(static_cast<bool>(ec));
    FsWorkerNotAuthUsr fs_worker4(std::move(root_path));
...
TEST_F(FixtureCreateFsNotAuthUsrInvalid, ENOENT) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/not_existing_dir/fs_sub_worker_not_auth_usr/");
...
TEST_F(FixtureCreateFsNotAuthUsrInvalid, ENAMETOOLONG) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH "/input_output" / fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x'));
...
TEST_F(FixtureCreateFsNotAuthUsrInvalid, ENOTDIR) {
    fs::path file_path = FS_WORKER_LIB_TEST_PATH "/input_output/file";
...
TEST(FsNotAuthUsr, MoveRoot) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/fs_sub_worker_not_auth_usr/");
    FsWorkerNotAuthUsr fs_worker((root_path));
    fs::path new_root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/fs_sub_worker_not_auth_usr_new/");
    fs_worker.move_root(new_root_path);
...
TEST_F(FixtureFsNotAuthUsrMoveRootInvalid, ENAMETOOLONG) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/fs_sub_worker_not_auth_usr/");
    fs::path new_root_path = FS_WORKER_LIB_TEST_PATH "/input_output" / fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x'));
...
TEST_F(FixtureFsNotAuthUsrMoveRootInvalid, ENOENT) {
    fs::path root_path = FS_WORKER_LIB_TEST_PATH/ fs::path("input_output/fs_sub_worker_not_auth_usr/");
    fs::path new_root_path = FS_WORKER_LIB_TEST_PATH / fs::path("input_output/not_existing_dir/fs_sub_worker_not_auth_usr/");
...
TEST(FsNotAuthUsr, MoveAndGetFile) {
    ...
    file << "This is fs_worker_lib of FsWorkerNotAuthUsr::move_file()";
    file.close();
