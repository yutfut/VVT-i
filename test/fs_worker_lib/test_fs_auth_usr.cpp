...
TEST(FsAuthUser, CreateValid) {
    fs::path root_path = TEST_PATH / fs::path("fs_sub_worker_auth_usr/");

    FsWorkerAuthUsr fs_worker1(root_path);
    EXPECT_EQ(root_path, fs_worker1.get_root());
    EXPECT_FALSE(fs_worker1.err_code);

    FsWorkerAuthUsr fs_worker2(fs_worker1);
    EXPECT_EQ(fs_worker2, fs_worker1);
    EXPECT_FALSE(fs_worker1.err_code);
    EXPECT_FALSE(fs_worker2.err_code);

    FsWorkerAuthUsr fs_worker3(std::move(fs_worker1));
    EXPECT_EQ(fs_worker3, fs_worker2);
    EXPECT_FALSE(fs_worker3.err_code);

    fs::remove_all(fs_worker3.get_root());
...
TEST_F(FixtureCreateFsAuthUserInvalid, ENOENT) {
    fs::path root_path = TEST_PATH / fs::path("not_existing_dir/fs_sub_worker_auth_usr/");
...
TEST_F(FixtureCreateFsAuthUserInvalid, ENAMETOOLONG) {
    fs::path root_path = TEST_PATH  / fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x'));
...
TEST(FsAuthUser, MoveRoot) {
    ...
    fs::path new_root_path = TEST_PATH / fs::path("fs_sub_worker_auth_usr_new/");
    fs_worker.move_root(new_root_path);
...
TEST_F(FixtureFsAuthUserMoveRootInvalid, ENAMETOOLONG) {
    fs::path root_path = TEST_PATH / fs::path("fs_sub_worker_auth_usr/");
    fs::path new_root_path = TEST_PATH / fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x'));
...
TEST_F(FixtureFsAuthUserMoveRootInvalid, ENOENT) {
    fs::path root_path = TEST_PATH / fs::path("fs_sub_worker_auth_usr/");
    fs::path new_root_path = TEST_PATH / fs::path("not_existing_dir/fs_sub_worker_auth_usr/");
...
TEST(FsAuthUser, WorkWithUsersFilesInvalidArgs) {
    ...
    EXPECT_TRUE(fs_worker.move_file(file_path, fname_moved, user));

    fs::path moved_file_path = root_path / user / fname_moved;
    EXPECT_TRUE(fs::equivalent(file_hlink_path, moved_file_path, ec));
    EXPECT_TRUE(fs_worker.get_file(fname_moved, user).is_open());

    // ENAMETOOLONG
    EXPECT_FALSE(fs_worker.move_file(file_hlink_path, fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x')), user));
    ...
    EXPECT_TRUE(fs_worker.remove("/not_existing_dir/file_move", user));
    EXPECT_FALSE(fs_worker.err_code);

    EXPECT_TRUE(fs_worker.remove(fname_moved, user));
    EXPECT_FALSE(fs::exists(moved_file_path, ec));
...
TEST(FsAuthUser, WorkWithUsersDirs) {
    ...
    EXPECT_TRUE(fs::exists(root_path /user/user_subdir_path, ec));
    EXPECT_FALSE(static_cast<bool>(fs_worker.err_code));
    ...
    fs::path fname_moved("moved_file");
    EXPECT_TRUE(fs_worker.move_file(file_path, user_subdir_path / fname_moved, user));
    EXPECT_TRUE(fs::equivalent(file_hlink_path, root_path / user / user_subdir_path / fname_moved, ec));
...
TEST(FsAuthUser, WorkWithUsersDirsInvalidArgs) {
    ...
    // ENAMETOOLONG
    EXPECT_FALSE(fs_worker.make_subdir(fs::path(std::string(static_cast<int>(fpathconf(0,_PC_NAME_MAX)) + 1, 'x')), user));
    EXPECT_EQ(fs_worker.err_code, std::errc(ENAMETOOLONG));
    ...
    EXPECT_TRUE(fs_worker.remove("not_existing_dir", user));
    EXPECT_FALSE(fs_worker.err_code);
